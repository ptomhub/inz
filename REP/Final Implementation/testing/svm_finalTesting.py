...
axe.scatter(X_test, predictions_for_test,facecolor = 'none', edgecolor='black', s=10, 
                     label='{} test data'.format('RBF'))
axe.scatter(X, model.predict(X), color='dodgerblue', s=50, 
                     label='{} model'.format('RBF'))
axe.legend(loc=2, bbox_to_anchor=(0.75, 1),
                     ncol=1, fancybox=False, shadow=True)

fig.text(0.5, 0.04, 'luminance [kcd/m^2]', ha='center', va='center')
